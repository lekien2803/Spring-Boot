...
-- cau4 
select c.id , json_arrayagg(json_object('id', b.id,
										'title', b.title,
										'description', b.description ,
	   									'thumbnail', b.thumbnail ,
	   									'published_at', DATE_FORMAT(b.published_at, '%d/%m/%y'),
	   									'author', JSON_OBJECT('id', u.id,
	   														  'name', u.name)
-- 	   									'count_comment', COUNT(c2.id) 
										)
							) as blog
FROM blog b 
left join blog_categories bc on bc.blog_id = b.id 
left JOIN category c on c.id = bc.categories_id 
INNER JOIN comment c2 on c2.blog_id = b.id 
LEFT JOIN `user` u on u.id = b.user_id 
WHERE c.id = 1

SELECT c.id, json_object('abc',json_object('title', b.title), 'count', COUNT(c2.id))
FROM blog b 
left join blog_categories bc on bc.blog_id = b.id 
left JOIN category c on c.id = bc.categories_id 
INNER JOIN comment c2 on c2.blog_id = b.id 
